...
// Protege a OPENAI_API_KEY (definir em Settings → Environment variables do projeto).
// Recebe { items: [{ id, text }] } e devolve { results: [{ id, cat }] }.

const CATEGORIAS = [
  'PONTO',            // ajustes/irregularidades no registro de ponto
  'ATRASO',           // atrasos e não comunicação de atraso
  'FALTA',            // ausência, não comparecimento
  'CONDUTA',          // comportamento inadequado, postura, dormir em serviço
  'INSUBORDINACAO',   // recusa de ordem, desrespeito à gestão
  'ERRO OPERACIONAL', // erros de processo, agendamento, cadastro, OS
  'PREJUIZO',         // dano financeiro ou material à empresa
  'ATENDIMENTO',      // falha com cliente, pesquisa da montadora, CSI
  'USO INDEVIDO',     // uso indevido de veículo, senha, sistema ou recurso
  'SEGURANCA',        // norma de segurança, direção perigosa
  'CORRECAO ADMINISTRATIVA', // pedido de correção de nome, prazo, parcela, arquivo
  'OUTROS'
];
...
Responda SOMENTE com JSON no formato {"results":[{"id":"...","cat":"..."}]} sem texto adicional.`;
  ...
  const user = JSON.stringify(lista);
